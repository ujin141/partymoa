...
`TARGETED_DEVICE_FAMILY` 를 `"1,2"` 로 바꿨다 (2026-09-04). 아이폰 전용
(`1`) 이던 것을 아이패드까지 켠 것이다.

  · 아이콘은 universal 1024 하나라 그대로 쓴다
  · 스플래시는 2732x2732 정사각 `scaleAspectFill` 이라 방향을 안 탄다
  · Info.plist 에 아이패드 4방향이 이미 선언돼 있다
  · 화면은 `max-w-[430px]` 칼럼이 회색(#E9EBEF) 위에 가운데로 선다.
    세로 834x1194, 가로 1194x834 에서 깨지지 않는 것을 확인했다

**App Store Connect 에 아이패드 13인치 스크린샷을 올려야 한다.**
`store/ios/upload/ipad13/` 에 2064 x 2752 로 다섯 장 있다. 없으면 제출
버튼이 안 눌린다.

`UIRequiresFullScreen` 은 안 걸었다 — 걸면 Split View 를 포기하는 것이고,
지금 레이아웃은 어떤 폭에서도 서기 때문에 걸 이유가 없다.
